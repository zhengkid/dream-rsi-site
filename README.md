...
**补丁（2026-09-15）：一个版本的分数是期望，不是一次抽样。**
`rollout()` 从一开始就重放子树，但 `addRevision()` 只跑一次 `rollout` 就给版本打分。
一次抽样里，「只开一条分支、恰好蒙对」总是打赢「开两条、稳定找到」——
因为多开的那条要多付 λ·n，而蒙对的那次不用付。结果是**胜出的版本几乎永远是单链**，
第 4 幕 Update policy 那张收口图画出来就是一条虚线路径，跟图注里写的 subtree 对不上。
改法：`evalPolicy()` 跑 `EK = 24` 次重放取**均值**当分数（论文里也是拿一个版本在整段
历史上重放来评分的），画出来的那棵子树取**最接近均值**的那次重放——代表性的，不是最好的那次。
`seedHistory()` 里挑 carry policy 也走同一个函数（900 次单抽 → 40 个候选 × 24 次重放）。
这一改之后宽度才真的开始有收益：贪心 = 开得少钻得深，稳健 = 开 2~3 条,收口图就是子树。
